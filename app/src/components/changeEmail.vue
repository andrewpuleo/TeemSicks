        ...
        <div class="modal-card">
            <div class="modal-content">
                <header class="modal-card-head">
                    <p class="modal-card-title"> Edit Email </p>
                </header>
                <section class="modal-card-body">
                    <p class="error-message-this" v-if="!same"> Inputs must be the same! <br></p>
                    <input class="input" type="text" placeholder="New Email" v-model="input1"><br>
                    <br>
                    <input class="input" type="text" placeholder="Confirm New Email" v-model="input2"><br>
                </section>
                <footer class="modal-card-foot">
                    <div class="button-positions">
                        <button class="button-save-changes" v-on:click="checkInputs(), changeEmail()"> Save changes </button>
                        &nbsp;&nbsp;&nbsp;&nbsp;
...
export default class ChangeEmail extends Vue{
    ...
    changeEmail(){
        let status = true;
        if(this.same){
            axios.put(`/api/users/${this.$store.getters.getUID}`, {...this.visitor,  email: this.input1}).then((res) => {
                this.visitor = res.data;
                this.$emit('onSuccessEmail', res.data.email);
                this.$emit('close', res.data.email);
